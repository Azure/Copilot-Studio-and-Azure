...
## Private Endpoint for Content Understanding Accelerator

The [Private Endpoint for Content Understanding Accelerator](/accelerators/private-endpoint/) hosts **Azure AI Content Understanding** behind a **Private Endpoint** with `publicNetworkAccess=Disabled` and lets a **Power Platform Managed Environment** reach it through **Enterprise Policy / VNet injection**. The accelerator ships a one-click ARM/Bicep deployment that provisions the AI Services account (kind `AIServices`, custom subdomain), the Private Endpoint, the three required Private DNS zones (`privatelink.cognitiveservices.azure.com`, `privatelink.openai.azure.com`, `privatelink.services.ai.azure.com`), two delegated subnets in paired Azure regions, and a `Microsoft.PowerPlatform/enterprisePolicies` (kind `vnet`) resource — plus PowerShell helpers to link the policy to a PP environment, push a ready-made custom connector for the Content Understanding REST API, and run end-to-end connectivity tests from your laptop, an in-VNet VM, and a Power Automate flow.

**Why this accelerator?** Azure AI Services lets you set `publicNetworkAccess=Disabled`, but Power Platform is **not** a "trusted Microsoft service" for Cognitive Services — `bypass=AzureServices` alone won't let a Power Automate flow reach a private-endpoint-locked AI Services account. The supported alternative is a Power Platform Enterprise Policy linked to a delegated subnet, but wiring it correctly (paired regions for multi-region PP geos, the right Private DNS zones, the `enterprisePolicies (vnet)` resource location, the PP-side link step that ARM cannot perform) is an easy place to get stuck for a week. This accelerator turns that work into a one-click deploy + a single linking script, with built-in tests that distinguish a working private endpoint, a working DNS resolution, and a working PP → PE path — so a team can stand up a private, network-locked Content Understanding endpoint that's reachable from Power Platform in an afternoon instead of a sprint.

Key capabilities:

- **One-click ARM deploy.** `infra/azuredeploy.json` (compiled from `infra/deploy.bicep`) provisions VNet, paired delegated subnet in a secondary region, AI Services account, Private Endpoint, three Private DNS zones, and the Enterprise Policy in a single template.
- **Scripted alternative.** `scripts/deploy.ps1` + `.env` for teams that prefer a `.env`-driven PowerShell flow over portal parameters.
- **Final-mile policy link.** `scripts/link-enterprise-policy.ps1` runs the Power Platform admin call that ARM can't, with `-UseDeviceCode` and `-Unlink` switches for clean re-deploys.
- **Custom connector + tests.** `powerplatform/contentunderstanding-connector.swagger.json` + `apiProperties.json` are pushed via `pac` by `scripts/create-and-test-connector.ps1`, which also calls `GET /contentunderstanding/analyzers` from your laptop (expect `403 Public access is disabled`) and from inside the VNet (expect `200 OK`).
- **Multi-geo region pairing.** Default parameters cover `westus`/`eastus` (`unitedstates`), `swedencentral`/`swedensouth` (`europe`), and `australiaeast`/`australiasoutheast` (`australia`) — pick a row, redeploy, no template editing.
- **Troubleshooting playbook.** README documents the common failure modes (`404` from the link call when the env isn't yet a Managed Environment, geo/region mismatches on the policy, `EnterprisePolicyUpdateNotAllowed` on re-deploy, cached `403 ThrowExceptionDueToTrafficDenied` connections in PP) with the exact remedy for each.
