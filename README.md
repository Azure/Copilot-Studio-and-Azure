...
- The [Voice (Push-to-Talk) Accelerator](/accelerators/voice-pushtotalk) gives a Copilot Studio agent a voice-capable Microsoft 365 / Teams surface **without Omnichannel / Contact Center licensing** and **without any custom hosting**. A Foundry Agent Service "IT Assistant" is published to Teams + M365 Copilot via the native [publish-copilot](https://learn.microsoft.com/azure/foundry/agents/how-to/publish-copilot) flow; it calls a Copilot Studio "Microsoft Learn Assistant" (grounded on the Microsoft Learn MCP server) over Direct Line via a custom OpenAPI tool.
